...
const ItemAlimento: React.FC<ItemAlimentoProps> = ({ alimentos }) => {
    return (
        <div className="w-full">
            ...
            <div className="grid grid-cols-5 bg-zinc-800 text-zinc-200 font-bold py-2 text-center border border-black">
                <div>Descrição</div>
                <div>Calorias (Kcal)</div>
            ...
            {alimentos.map((alimento, index) => (
                <div
                    key={index}
                    className={`grid grid-cols-5 text-center py-2 border border-black ${
                        index % 2 === 0 ? 'bg-lime-50' : 'bg-lime-200'
                    }`}
                >
                    <div className='border-r-2 border-black'>{alimento.descricao}</div>
                    <div className='border-r-2 border-black'>{alimento.energia}</div>
